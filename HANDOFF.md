...
## Status
v35 · Sep 5 · previous main head 587bea7. Single-file PWA: index.html (~2,025 lines) + sf-icon.png + design/sf-move-spend-mockup.html. All state in localStorage key sfMoveApp_v1. Served from epicminds-eng.github.io/sf-move per the in-app Shortcut recipe (Pages deployment unverified this session).
...
- Trip: option-B SVG map with real state outlines (:385), everything inside <g id="zoomLayer"> (:401) with pinch-zoom 1–5× + pan (module :1514–:1615: Pointer Events, rubber-band, momentum → critically damped rAF springs, double-tap/Reset pill reset, counter-scaled nodes/labels/live dot, touch-action pan-y at 1× and none while zoomed, reset on tab change, never persisted); stops St. Robert → Weatherford → Albuquerque → Mom's → Seychelle's LA → 1442A Grove St SF (STOPS :1311, BACKUPS :1339, WAYPTS :1359 incl. a Lost Hills I-5 waypoint); road-following live tracking (locate :1407); CHARGES Supercharger log (:1355, code-seeded; shape in the comment) → Tesla-red #E82127 nodes on the map + a tap-to-expand "Charging" row under the mileage line (renderCharging :1488, red dot icons, Apple Maps links per session). planned:true entries (today: chg-oasis, Tesla Oasis Lost Hills) draw hollow, list as "Planned · name", never touch Spend or totals; swap the object in place for a real session and it fills in + seeds Spend; trip.departDate is the single source for every date (default 2026-09-06, migrations V1 09-04→09-05 and V2 09-05→09-06 at :799–:808, each flagged once and never re-applied); Orbitz links derive their dates (:827).
- Spend: planned/actual entries, per-day add forms with the current day open, projection + budget, seeded planned entries (:1610), CHARGES auto-seeded as actual charging entries with id = charge id (seedCharges :1901, skips planned; deleting one records spend.dismissedCharges[id] so it never comes back), URL ingest ?add=&m= and paste import with a merchant table (:1915–:1999).
...
## Where things live
- State init + migrations: index.html:780–822 (departDate default + V1/V2 flags :799–:808, retired-stop arrived reset :832, spend :833; spend.dismissedCharges is created lazily on first delete).
- Renderers: renderMove :863, renderSort :942, renderBeforeLeave :1108, renderPack :1153, Trip :1310, map zoom :1514, Spend :1729, tabs :2002.
