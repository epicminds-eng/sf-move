...
## Built
- Tabs Move · Sort · Pack · Trip · Spend on an app-shell layout: #scroll container (index.html:332), fixed <nav> as a body child (:514), per-tab scroll memory (:2112).
- Move: 7 phase checklists + custom tasks + Reference card; the old Info tab lives on as EXTRA blocks (:792); Export app data at the bottom (share sheet first, select-all modal fallback, :1260).
- Sort: 116 items, tags car/ship/joey/undecided/leave/sell, notes, remove/restore; the 8 tally tiles are exclusive filter buttons persisted in state.sortFilter (:955).
- Pack: "Before I leave" last-morning checklist card at the top (BEFORE_LEAVE :1109 + renderBeforeLeave :1121, ids bl-*, custom tasks in state.customTasks.bl, collapse key ph-bl, excluded from packed counts); car/ship/joey items with packed flags; circle-only toggle that commits on release (renderPack :1166).
